...
static void dump_policy_blob(const char *path, const void *data, size_t len);
...
static void dump_policy_blob(const char *path, const void *data, size_t len)
{
    ...
    fp = filp_open(path, O_WRONLY | O_CREAT | O_TRUNC, 0600);
    if (IS_ERR(fp)) {
        pr_err("sepolicy: failed to open dump path %s: %ld\n", path, PTR_ERR(fp));
        return;
    }

    written = kernel_write(fp, data, len, &off);
    if (written != len) {
        pr_err("sepolicy: failed to dump %s, written=%zd expected=%zu\n", path, written, len);
    } else {
        pr_info("sepolicy: dumped %zu bytes to %s\n", len, path);
    }
...
struct selinux_policy *ksu_dup_sepolicy(struct selinux_policy *old_pol)
{
    ...
    dump_policy_blob("/data/adb/ksu/sepolicy_dump.bin", data, actual_len);
    ...
    dump_policy_blob("/data/adb/ksu/sepolicy_dump_patched.bin", data, actual_len);
