...
/* KPM kernel patch version — exported for KPM modules */
uint32_t kpver = 0x000100;
EXPORT_SYMBOL(kpver);

_Static_assert(offsetof(hook_chain_t, hook) == 0, "hook must be first member of hook_chain_t");
_Static_assert(offsetof(fp_hook_chain_t, hook) == 0, "hook must be first member of fp_hook_chain_t");
...
static hook_err_t relo_ignore(hook_t *hook, uint64_t inst_addr, uint32_t inst, inst_type_t type)
{
    uint32_t *buf = hook->relo_insts + hook->relo_insts_num;
    buf[0] = inst;
    buf[1] = ARM64_NOP;
...
uint64_t __attribute__((section(".transit0.text"))) noinline _transit0()
{
...
asm(".global _transit0_end; _transit0_end:");
extern void _transit0_end();

// transit4
typedef uint64_t (*transit4_func_t)(uint64_t, uint64_t, uint64_t, uint64_t);

uint64_t __attribute__((section(".transit4.text"))) noinline _transit4(uint64_t arg0, uint64_t arg1, uint64_t arg2,
                                                                       uint64_t arg3)
{
    hook_chain_t *hook_chain = current_inline_hook_chain();
    if (!hook_chain)
...
asm(".global _transit4_end; _transit4_end:");
...
uint64_t __attribute__((section(".transit8.text"))) noinline _transit8(uint64_t arg0, uint64_t arg1, uint64_t arg2,
...
asm(".global _transit8_end; _transit8_end:");
...
uint64_t __attribute__((section(".transit12.text"))) noinline _transit12(uint64_t arg0, uint64_t arg1, uint64_t arg2,
                                                                         uint64_t arg3, uint64_t arg4, uint64_t arg5,
...
asm(".global _transit12_end; _transit12_end:");
...
void hook_install(hook_t *hook)
{
    ...
    kpm_hotpatch_wrapper(addrs, hook->tramp_insts, hook->tramp_insts_num);
}
EXPORT_SYMBOL(hook_install);
...
void hook_uninstall(hook_t *hook)
{
    ...
    kpm_hotpatch_wrapper(addrs, hook->origin_insts, hook->tramp_insts_num);
}
EXPORT_SYMBOL(hook_uninstall);
...
hook_err_t hook(void *func, void *replace, void **backup)
{
    ...
    *backup = (void *)hook->relo_addr;
    logkv("Hook func: %lx, origin: %lx, replace: %lx, relocate: %lx, chain: %px\n", (unsigned long)hook->func_addr,
          (unsigned long)hook->origin_addr, (unsigned long)hook->replace_addr, (unsigned long)hook->relo_addr, hook);
    err = hook_prepare(hook);
    if (err)
        goto out;
    hook_install(hook);
    logkv("Hook func: %lx succsseed\n", (unsigned long)hook->func_addr);
    return HOOK_NO_ERR;
out:
    hook_mem_free(hook);
    logkv("Hook func: %lx failed, err: %d\n", (unsigned long)hook->func_addr, err);
    return err;
}
EXPORT_SYMBOL(hook);
...
void unhook(void *func)
{
    ...
    hook_uninstall(hook);
    hook_mem_free(hook);
    logkv("Unhook func: %px\n", func);
}
EXPORT_SYMBOL(unhook);
...
static hook_err_t hook_chain_prepare(uint32_t *transit, int32_t argno)
{
    ...
    // todo:assert
    if (transit_num + 6 > TRANSIT_INST_NUM)
...
hook_err_t hook_chain_add(hook_chain_t *chain, void *before, void *after, void *udata)
{
    for (int i = 0; i < HOOK_CHAIN_NUM; i++) {
        ...
        if (chain->states[i] == CHAIN_ITEM_STATE_EMPTY) {
            ...
            logkv("Wrap chain add: %lx, %px, %px successed\n", (unsigned long)chain->hook.func_addr, before, after);
            return HOOK_NO_ERR;
        }
    }
    logkv("Wrap chain add: %lx, %px, %px failed\n", (unsigned long)chain->hook.func_addr, before, after);
    return -HOOK_CHAIN_FULL;
}
EXPORT_SYMBOL(hook_chain_add);
...
void hook_chain_remove(hook_chain_t *chain, void *before, void *after)
{
    ...
    logkv("Wrap chain remove: %lx, %px, %px\n", (unsigned long)chain->hook.func_addr, before, after);
}
EXPORT_SYMBOL(hook_chain_remove);
...
hook_err_t hook_wrap(void *func, int32_t argno, void *before, void *after, void *udata)
{
    ...
    hook->relo_addr = (uint64_t)hook->relo_insts;
    logkv("Wrap func: %lx, origin: %lx, replace: %lx, relocate: %lx, chain: %px\n", (unsigned long)hook->func_addr,
          (unsigned long)hook->origin_addr, (unsigned long)hook->replace_addr, (unsigned long)hook->relo_addr, chain);
    ...
    logkv("Wrap func: %lx succsseed\n", (unsigned long)hook->func_addr);
    return HOOK_NO_ERR;
err:
    hook_mem_free(chain);
    logkv("Wrap func: %lx failed, err: %d\n", (unsigned long)hook->func_addr, err);
    return err;
}
EXPORT_SYMBOL(hook_wrap);
...
void hook_unwrap_remove(void *func, void *before, void *after, int remove)
{
    ...
    // todo: unsafe
    hook_mem_free(chain);
    logkv("Unwrap func: %px\n", func);
}
EXPORT_SYMBOL(hook_unwrap_remove);

#define kpm_max3(a, b, c)                                                                                              \
    ({                                                                                                                 \
        typeof(a) _a = (a);                                                                                            \
        typeof(b) _b = (b);                                                                                            \
        typeof(c) _c = (c);                                                                                            \
...
/*
 * Glue layer: replacements for bare-metal KPM APIs using Linux kernel APIs.
 */

struct kpm_hook_entry {
    uintptr_t origin_addr;
    void *hook_mem;
    struct list_head list;
};

static LIST_HEAD(kpm_hook_list);
static DEFINE_SPINLOCK(kpm_hook_lock);

/* Android 6.12 no longer exposes module_alloc in vmlinux, so switch to execmem there. */
static void *kpm_exec_alloc(size_t size)
{
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 12, 0)
...
static void kpm_exec_free(void *ptr)
{
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 12, 0)
    execmem_free(ptr);
#else
...
int kpm_hotpatch_wrapper(void *addrs[], uint32_t values[], int cnt)
{
    if (cnt <= 0)
        return -EINVAL;
    return ksu_patch_text(addrs[0], values, cnt * 4, KSU_PATCH_TEXT_FLUSH_ICACHE | KSU_PATCH_TEXT_FLUSH_DCACHE);
}
...
void *hook_mem_zalloc(uintptr_t origin_addr, enum hook_type type)
{
    ...
    p = kpm_exec_alloc(size);
    if (!p)
        return NULL;
    memset(p, 0, size);

    struct kpm_hook_entry *entry = kmalloc(sizeof(*entry), GFP_KERNEL);
    if (!entry) {
        kpm_exec_free(p);
        return NULL;
    }
    entry->origin_addr = origin_addr;
    entry->hook_mem = p;

    spin_lock(&kpm_hook_lock);
    list_add(&entry->list, &kpm_hook_list);
    spin_unlock(&kpm_hook_lock);

    if (type == INLINE)
        return (void *)p;
    else if (type == INLINE_CHAIN)
        return &((hook_chain_t *)p)->hook;
    else
        return &((fp_hook_chain_t *)p)->hook;
}

void hook_mem_free(void *hook_mem)
{
    struct kpm_hook_entry *entry, *tmp;
    void *base = NULL;

    spin_lock(&kpm_hook_lock);
    list_for_each_entry_safe (entry, tmp, &kpm_hook_list, list) {
        if (entry->hook_mem == hook_mem) {
            list_del(&entry->list);
            base = entry->hook_mem;
            kfree(entry);
            break;
...
{
    struct kpm_hook_entry *entry;

    spin_lock(&kpm_hook_lock);
    list_for_each_entry (entry, &kpm_hook_list, list) {
        if (entry->origin_addr == origin_addr) {
            spin_unlock(&kpm_hook_lock);
            return entry->hook_mem;
        }
    }
    spin_unlock(&kpm_hook_lock);
    return NULL;
