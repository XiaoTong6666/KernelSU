...
uint64_t __attribute__((section(".fp.transit0.text"))) noinline _fp_transit0()
{
...
asm(".global _fp_transit0_end; _fp_transit0_end:");
extern void _fp_transit0_end();

// transit4
typedef uint64_t (*transit4_func_t)(uint64_t, uint64_t, uint64_t, uint64_t);

uint64_t __attribute__((section(".fp.transit4.text"))) noinline _fp_transit4(uint64_t arg0, uint64_t arg1,
...
asm(".global _fp_transit4_end; _fp_transit4_end:");
...
uint64_t __attribute__((section(".fp.transit8.text"))) noinline _fp_transit8(uint64_t arg0, uint64_t arg1,
                                                                             uint64_t arg2, uint64_t arg3,
...
asm(".global _fp_transit8_end; _fp_transit8_end:");
...
uint64_t __attribute__((section(".fp.transit12.text"))) noinline
...
asm(".global _fp_transit12_end; _fp_transit12_end:");
...
void fp_hook(uintptr_t fp_addr, void *replace, void **backup)
{
    *(uintptr_t *)backup = *(uintptr_t *)fp_addr;
    ...
    addrs[0] = fp_addr;
    addrs[1] = fp_addr + 4;
    kpm_hotpatch_wrapper((void **)addrs, (uint32_t *)&replace, 2);
}
EXPORT_SYMBOL(fp_hook);
...
void fp_unhook(uintptr_t fp_addr, void *backup)
{
    ...
    addrs[0] = fp_addr;
    addrs[1] = fp_addr + 4;
    kpm_hotpatch_wrapper((void **)addrs, (uint32_t *)&backup, 2);
}
EXPORT_SYMBOL(fp_unhook);
...
hook_err_t fp_hook_wrap(uintptr_t fp_addr, int32_t argno, void *before, void *after, void *udata)
{
    ...
    for (int i = 0; i < FP_HOOK_CHAIN_NUM; i++) {
        ...
        if (chain->states[i] == CHAIN_ITEM_STATE_EMPTY) {
            ...
            logkv("Wrap func pointer add: %lx, %px, %px successed\n", (unsigned long)chain->hook.fp_addr, before,
                  after);
            return HOOK_NO_ERR;
        }
    }
    logkv("Wrap func pointer add: %lx, %px, %px failed\n", (unsigned long)chain->hook.fp_addr, before, after);
    return -HOOK_CHAIN_FULL;
}
EXPORT_SYMBOL(fp_hook_wrap);
...
void fp_hook_unwrap(uintptr_t fp_addr, void *before, void *after)
{
    ...
    logkv("Wrap func pointer remove: %lx, %px, %px\n", (unsigned long)chain->hook.fp_addr, before, after);
    ...
    // todo: unsafe
    hook_mem_free(chain);
    logkv("Unwrap func pointer: %lx, %px, %px\n", (unsigned long)fp_addr, before, after);
}
EXPORT_SYMBOL(fp_hook_unwrap);
